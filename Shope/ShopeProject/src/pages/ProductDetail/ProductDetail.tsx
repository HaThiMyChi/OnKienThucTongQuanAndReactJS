...
export default function ProductDetail() {
  ...
  // Dùng React Query để lấy 20 sản phẩm cùng category với sản phẩm hiện tại.
  // Nó chỉ gọi API khi product đã có dữ liệu nhờ enabled: Boolean(product), và cache kết quả trong 3 phút nhờ staleTime.
  ...
  const next = () => {
    // currentIndexImages[1] = Nó là điểm dừng / giới hạn kết thúc cho slice
    // [0, 5] nghĩa là lấy từ 0 đến trước 5 => ảnh hiển thị thật sự là 0,1,2,3,4

    console.log('currentIndexImages[1]', currentIndexImages[1])
  ...
  const handleZoom = (event: React.MouseEvent<HTMLDivElement, MouseEvent>) => {
    ...
    // naturalWidth và naturalHeight là kích thước thực tế của file ảnh
          ...
          <div className='grid grid-cols-12 gap-9'>
            <div className='col-span-5'>
              ...
              <div className='relative mt-4 grid grid-cols-5 gap-1'>
                <button
                  className='absolute left-0 top-1/2 z-10 h-9 w-5 -translate-y-1/2 bg-black/20 text-white disabled:opacity-50'
                  onClick={prev}
                ...
                <button
                  className='absolute right-0 top-1/2 z-10 h-9 w-5 -translate-y-1/2 bg-black/20 text-white disabled:opacity-50'
                  onClick={next}
