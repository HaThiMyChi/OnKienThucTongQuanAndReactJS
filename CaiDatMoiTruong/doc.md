...
##. Khởi tạo Project với CRA

## 3. Giới thiệu về cấu trúc folder Project React.Js do CRA tạo
...
Nếu như các bạn muốn setting mỗi workpace hiện tại thôi (nghĩa là cài folder hiện tại) thì các bạn setting bên workspace
...
- `Prettier` là một công cụ tự động định dạng mã nguồn theo các quy tắc rõ ràng và đơn giản. Nó sẽ tự động sửa đổi các lỗi định dạng như các dòng quá dài, khoảng trắng thừa, ...
...
Trên là những plugin cần thiết để setting Prettier về ESlint cho CRA. Các bạn nếu dùng yarn thì chạy `yarn add` nhé.

### 4.5. Tạo file `.eslintrc` để setting eslint

-.`eslintrc`: Nó cấu hình các tùy chọn cho công cụ ESlint.
- `ESLint` là một công cụ kiểm tra chất lượng mã nguồn. Nó sẽ phát hiện các lỗi cú pháp, các từ khóa trùng lặp.
...
Cú pháp viết trong những file này tương tự như trong file `.gitignore`, các bạn có thể tìm lại trong gile `GitCanban.md` trong thư mục `GitCanBanDenNangCao` nhé.
