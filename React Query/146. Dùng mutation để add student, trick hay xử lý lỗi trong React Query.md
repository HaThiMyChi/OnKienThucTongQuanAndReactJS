...
- Tiếp theo, `formData.entries()` trả về một iterator chứa tất cả cặp khóa-giá trị (key-value) trong form data. `Object.fromEntries()` được sử dụng để chuyển đổi iterator này thành một đối tượng JSON
  `newStudent`, trong đó khóa là tên của các trường form và giá trị là giá trị đã nhập.
