...
- Trong đó:
  ...
  - Kiểu AppDispatch là một kiểu dữ liệu có chứa kiểu của hàm store.dispatch. Ta sử dụng nó để định nghĩa cho hook useDispatch.
  - Hook useAppDispatch sử dụng useDispatch từ module react-redux để trả về một tham chiếu tới dispatch của store. Chúng ta cũng định nghĩa kiểu cho useDispatch bằng cách sử dụng AppDispatch
...
Trong đoạn code trên, chúng ta đang sử dụng hàm configureStore được cung cấp bởi Redux Toolkit để tạo một Redux store. Hàm này nhận vào một đối tượng configuration object với các trường hợp sau:

- reducers: Đây là một đối tượng chứa các reducers, tương tự như một store Redux truyền thống. Tuy nhiên, thay vì phải tự viết reducer chúng ta có thể sử dụng createSlice của Redux Toolkit để tạo reducer một cách nhanh chóng và đơn giản hơn. Trong đoạn code này, chúng ta chỉ có một reducer duy nhất là blogReducer, và nó được đặt tên là `blog`
